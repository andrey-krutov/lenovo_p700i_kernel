								  ...
								  IMG_BOOL			bFlush)
{
		...
		||	(ui32ReadOpsComplete != psWalkerObj->ui32ReadOps2Pending))
		{
				...
				!SYNCOPS_STALE(ui32ReadOpsComplete, psWalkerObj->ui32ReadOps2Pending))
			{
				PVR_DPF((PVR_DBG_ERROR,
							"PVRSRVProcessCommand: DstWalkerObj=%p, WOP=%d ROP=%d ROP2=%d fail dependencies",
							psWalkerObj, 
	...
	while (psWalkerObj < psEndObj)
	{
		...
		|| (ui32ReadOpsComplete != psWalkerObj->ui32ReadOps2Pending))
		{
				...
				!SYNCOPS_STALE(ui32ReadOpsComplete, psWalkerObj->ui32ReadOps2Pending))
			{
				...
				{
					PVR_DPF((PVR_DBG_ERROR,
								"PVRSRVProcessCommand: SrcWalkerObj=%p, WOP=%d ROP=%d ROP2=%d fail dependencies",
								psWalkerObj, 
								psWalkerObj->ui32WriteOpsPending,
								psWalkerObj->ui32ReadOpsPending,
								psWalkerObj->ui32ReadOps2Pending));
	...
	if (psCmdCompleteData->bInUse)
	{
		PVR_DPF((PVR_DBG_ERROR,	"PVRSRVProcessCommand: psCmdCompleteData=%p InUse", psCmdCompleteData));
		return PVRSRV_ERROR_FAILED_DEPENDENCIES;
	}
	...
	{
		PVR_DPF((PVR_DBG_ERROR, "PVRSRVProcessCommand: psCmdCompleteData=%p not processed", psCmdCompleteData));
...
PVRSRV_ERROR PVRSRVProcessQueues(IMG_BOOL	bFlush)
{
	...
	while (OSLockResource(&psSysData->sQProcessResource, ISR_ID) != PVRSRV_OK)
	{
		...
		if (iTry == 0)
		{
			PVR_DPF((PVR_DBG_ERROR,"PVRSRVProcessQueues LockResource forever?"));	
		}
#endif
	};
	...
	while (psQueue)
	{
		...
		if (iTry == 0)
		{
			PVR_DPF((PVR_DBG_ERROR,"PVRSRVProcessQueues PVRSRVProcessCommand forever?"));	
		}
#endif
	}
