...
/* lenovo-sw, add by zhouwl, 2012-6-19, add for line control earphone */
#ifdef LENOVO_LINECONTROL_EARPHONE
extern struct input_dev *kpd_input_dev;
extern unsigned int Get_Adc_Channel1Value(void);
#endif
/* lenovo-sw, add by zhouwl, 2012-6-19, add for line control earphone */
...
/* lenovo-sw, add by zhouwl, 2012-6-19, add for line control earphone */
#ifdef LENOVO_LINECONTROL_EARPHONE
		value = Get_Adc_Channel1Value();
	    	ACCDET_DEBUG("[zhouwl-Adc]Get_Adc1Value = %d\n", value);
		if(is_long_press())
		{
			if((value >= 250) && (value <=290)) //vol-
			{
				input_report_key(kpd_input_dev, KEY_PREVIOUSSONG, 1);
				input_report_key(kpd_input_dev, KEY_PREVIOUSSONG, 0);
	                	input_sync(kpd_input_dev);
			}
			else if((value >= 110) && (value <=150)) //vol+
			{
				input_report_key(kpd_input_dev, KEY_NEXTSONG, 1);
				input_report_key(kpd_input_dev, KEY_NEXTSONG, 0);
	                	input_sync(kpd_input_dev);
			}
			else if(value <=20) //play&pause
			{
				if(call_status == 0)
				{
...
#endif /* LENOVO_LINECONTROL_EARPHONE */	
/* lenovo-sw, add by zhouwl, 2012-6-19, add for line control earphone */
