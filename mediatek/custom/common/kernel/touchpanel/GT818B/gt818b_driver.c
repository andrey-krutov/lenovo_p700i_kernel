...
extern s32 init_wr_nsode(struct i2c_client*);
...
#define GT818B_TPD_VELOCITY_CUSTOM_X 10
#endif
#ifndef TPD_VELOCITY_CUSTOM_Y
#define GT818B_TPD_VELOCITY_CUSTOM_Y 10
#endif
...
int gt818b_g_v_magnify_x =GT818B_TPD_VELOCITY_CUSTOM_X;
int gt818b_g_v_magnify_y =GT818B_TPD_VELOCITY_CUSTOM_Y;
       ...
       unsigned long arg)
{
	...
	switch(cmd)
	{
		case TPD_GET_VELOCITY_CUSTOM_X:
			...
			if(copy_to_user(data, &gt818b_g_v_magnify_x, sizeof(gt818b_g_v_magnify_x)))
			{
				err = -EFAULT;
				break;
			}				 
	   ...
	   case TPD_GET_VELOCITY_CUSTOM_Y:
			...
			if(copy_to_user(data, &gt818b_g_v_magnify_y, sizeof(gt818b_g_v_magnify_y)))
			{
				err = -EFAULT;
				break;
			}				 
...
#ifdef HAVE_TOUCH_KEY
	for(retry = 0; retry < TPD_TOUCH_KEY_NUM; retry++)
	{
		input_set_capability(tpd->dev,EV_KEY,touchKeyArray[retry]);	
	}
#endif
...
/*for tp edging -- by molg1.lenovo 6.27.2012 start*/
#if defined(LENOVO_PROJECT_A570)
		if(wrap_x > 408)
		{
			wrap_x -= (wrap_x - 408) * 11 / 68;
		}
		else if(wrap_x < 70)
		{
			wrap_x += (70 - wrap_x) * 11 / 68;
		}
		if(wrap_y > 720)
		{
			wrap_y -= (wrap_y - 720) * 11 / 77;
		}
		else if(wrap_y < 80)
		{
			wrap_y += (80 - wrap_y) * 11 / 77;
		}
#endif
/*for tp edging  -- by molg1.lenovo 6.27.2012 end*/
 ...
 // linux kernel update from 2.6.35 --> 3.0
	...
	{
		for(count = 0; count < TPD_TOUCH_KEY_NUM; count++)
		{
			input_report_key(tpd->dev, touchKeyArray[count], !!(key&(0x01<<count)));
		}
	}		
	last_key = key;	
#endif
...
static void tpd_suspend( struct early_suspend *h )
{
    ...
    if(err < 0)
    {
        TPD_DMESG( "tpd_suspend write fail, can not suspend\n");
    }
    i2c_write_dummy( i2c_client, TPD_HANDSHAKING_END_REG );    

#ifdef MT6577//, using PMIC Power off leakage 0.3mA, sleep mode: 0.1mA    
/*lenovo-sw, zhouwl, 2012-06-01, do not close this power in suspend mode, cause i2c bus abnormal!!!*/
#if 0 
    hwPowerDown(MT65XX_POWER_LDO_VGP2, "TP");
    hwPowerDown(MT65XX_POWER_LDO_VGP, "TP");
#endif	
