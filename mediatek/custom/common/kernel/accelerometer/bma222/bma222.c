...
/*----------------------------------------------------------------------------*/
/* P700i use bma222e chip  -- liaoxl.lenovo 5.17.2012 start */
#ifdef LENOVO_PROJECT_P700I
	#define BMA222E_CHIP_EN
#endif
/* P700i use bma222e chip  -- liaoxl.lenovo 5.17.2012 end */
...
/* P700i use bma222e chip  -- liaoxl.lenovo 5.17.2012 start */
#ifdef BMA222E_CHIP_EN
static struct i2c_board_info __initdata i2c_BMA222={ I2C_BOARD_INFO("BMA222", (BMA222E_I2C_SLAVE_WRITE_ADDR>>1))};
#else
...
/* P700i use bma222e chip  -- liaoxl.lenovo 5.17.2012 start */
#ifdef BMA222E_CHIP_EN
int BMA222E_read_byte(struct i2c_client *client, u8 addr, u8 *data)
{
    u8 buf;
    int ret = 0;
    ...
    if (ret < 0) {
        GSE_ERR("reads data error!!\n");
        return -EFAULT;
    }
	else {
#if defined(HWMSEN_DEBUG)    
        GSE_LOG("%s(0x%02X) = %02X\n", __func__, addr, buf);    
#endif
    }
    *data = buf;
...
static ssize_t show_status_value(struct device_driver *ddri, char *buf)
{
	...
	{
#ifdef BMA222E_CHIP_EN
		err = BMA222E_read_byte(obj->client, i, regsbuf);
#else
		err = hwmsen_read_block(obj->client, i, regsbuf, 1);
#endif
		if (err != 0)
		{
			GSE_ERR("fail to read regs!\n");
			len += snprintf(buf+len, PAGE_SIZE-len, "fail to read reg[0x%x]!\n", i);
		}
		else
		{
